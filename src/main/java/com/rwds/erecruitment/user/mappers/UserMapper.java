...
public interface UserMapper {
    @Insert("INSERT INTO users ( namadepan, namabelakang, surel, katasandi, usia, telepon) VALUES ( #{user.namadepan}, #{user.namabelakang},#{user.surel},#{user.katasandi}, #{user.usia},#{user.telepon})")
    ...
    @Select("select * from users where surel = #{surel} AND id!=#{id}")
    User findBySurelNotPengguna(@Param("surel") String surel, @Param("id") Integer id);

    @Select("select * from users where id = #{id}")
    User findById(@Param("id") Integer id);

    @Select("select * from users where surel = #{surel} AND katasandi = #{katasandi}")
